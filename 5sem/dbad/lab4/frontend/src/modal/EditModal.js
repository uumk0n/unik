...
const EditModal = ({
  ...
  const renderInput = (columnName, columnType) => {
    ...
    if (columnType === "datetime") {
      const selectedDate = formData[lowercaseColumnName] || null;
      const dateWithoutTime = truncateTime(selectedDate);

      return (
        <DatePicker
          selected={dateWithoutTime}
          onChange={(date) => handleDateChange(lowercaseColumnName, date)}
      ...
      return (
        <select
          name={lowercaseColumnName}
          value={formData[lowercaseColumnName] || ""}
          onChange={(e) =>
            handleComboBoxChange(lowercaseColumnName, e.target.value)
          }
        >
      ...
      const options = comboBoxOptions[lowercaseColumnName] || [];
      return (
        <select
          name={lowercaseColumnName}
          value={formData[lowercaseColumnName] || ""}
      ...
      return (
        <input
          ...
          value={formData[lowercaseColumnName] || ""}
          onChange={(e) =>
            handleInputChange(lowercaseColumnName, e.target.value)
          }
        />
      );
    }

...
const AddModal = ({ isOpen, onClose, tableName, columnNames, tabs }) => {
            ...
            if (tab) {
              try {
                const optionsResponse = await axios.get(
                  `http://localhost:8000/get_combo_box_options/${tab.model}/${columnName}/`
                );

                optionsData[columnName.toLowerCase()] = optionsResponse.data;
  ...
  const renderInput = (columnName, columnType) => {
    if (columnType === "datetime") {
      ...
      isForeignKey(columnName,tableName)
    ) {
      const options = comboBoxOptions[columnName.toLowerCase()] || [];
      return (
        <select
          name={columnName}
          value={formData[columnName] || ""}
  ...
  const isForeignKey = (columnName,tableName) => {
    if(tableName.toLowerCase() === "edudocs"&& columnName.toLowerCase() === "regedudoc"){
      return true;
    }else if(tableName.toLowerCase() === "hfinfos" && columnName.toLowerCase() === "regorgnum"){
      return true;
    } else if (tableName.toLowerCase() === "personaldata" && columnName.toLowerCase() === "inila"){
      return true;
    }
    return false;
  };
  ...
  const handleComboBoxChange = (columnName, value) => {
    const parsedValue = value === 'true' || value === 'false' ? JSON.parse(value) : value;

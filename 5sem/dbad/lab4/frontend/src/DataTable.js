...
const DataTable = ({
  ...
  const handleGenerateReport = async (selectedTables) => {
    try {
      ...
      // Generate Excel with the connected table
      generateExcel(connectedTable, "output.xlsx");
      generatePDF(connectedTable, "output.pdf");

      //generateWord(data, templatePath, outputPath);
    } catch (error) {
  ...
  async function generateExcel(data, filePath) {
    // Check if the data arrays are not undefined and have a length
    if (!data || !data.length) {
    ...
    const combinedData = data[Object.keys(data)[0]].map((_, index) => {
      const combinedItem = {};

      // Flatten each table's data into the combined item
      Object.keys(data).forEach((tableName) => {
        const tableDataList = data[tableName];
        const tableData = tableDataList[index];

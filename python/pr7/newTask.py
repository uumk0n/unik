...
svm2 = LinearSVC(random_state=1, max_iter=1000)
...
plt.title('LinearSVC - 2 параметра')
...
plt.scatter(X_train[y_train == -1, 0], X_train[y_train == -1, 2], color='red', marker='o', label='setosa')
plt.xlabel('sepal length [cm]')
plt.ylabel('petal length [cm]')
plt.legend(loc='upper left')
plt.title('LinearSVC - 4 параметра')
...
plt.ylabel('petal length [cm]')
plt.legend(loc='upper left')
plt.title('Перцептрон - 4 параметра')
